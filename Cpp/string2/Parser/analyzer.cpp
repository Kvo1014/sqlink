...
Analyzer::Analyzer() : m_predTypes(predTypes, predTypes + sizeof(predTypes)/sizeof(string)), m_keyWords(keyWords, keyWords + sizeof(keyWords)/sizeof(string)), m_operators(operators, operators + sizeof(operators)/sizeof(string)), m_delimeters(delimeters, delimeters +sizeof(delimeters) sizeof(string))
{
    m_firstToken = true;
    m_type = false;
    m_ifElse = 0;
